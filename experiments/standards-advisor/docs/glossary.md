...
| **M0 – M3** | Delivery milestones (§9). M0 is the feasibility check; M1 end-to-end skeleton plus registry access; M2 profiler; M3 rank, explain, check and measure |
...
| **Feasibility gate** | One of the three M0 questions in §8.1, each mapping onto a stop condition |
...
| **Fixture** | A hand-made dataset with a deliberate fault and a known right answer, used as the regression suite and weight-sweep target (§8.2). "Should find nothing" is a valid right answer |
