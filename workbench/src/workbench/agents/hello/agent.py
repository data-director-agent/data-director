...
`../README.md` — a new input class, a new payload class, a grounding mode, a uischema fragment,
an entry point, a profile entry, a sample and marked tests — with no domain logic in the way.
...
from workbench.evidence import HASH_ALGORITHM, INPUT_CANONICALISATION
...
# Step 7 of the recipe: the RJSF fragment the shell composes under `payload`. Shipped with the
...
class HelloWorld:
    ...
    spec = AgentSpec(
        ...
        # requirement, so none is claimed here (recipe step 8).
        requirement_ids=(
            "DD-REGISTRY",
            "DD-INPUT-ACCEPTS",
            "DD-GROUNDING-MODE",
    ...
    def run(self, request: InvocationRequest, ctx: RunContext) -> AgentResult:
        ...
        return AgentResult(
            ...
            # Step 3: the canonicalisation is a registered name from `evidence.py` (ADR-0009),
...
def build(settings: Settings) -> HelloWorld:
    """The factory the entry point names (step 5). An agent that needed configuration would read
    its own `DD_HELLO_*` environment variables here, not in `settings.py`."""
    return HelloWorld()
